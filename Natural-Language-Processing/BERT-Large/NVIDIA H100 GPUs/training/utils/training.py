    ...
    if not args.skip_train:
        ...
        if args.do_train and args.train_iters > 0:
            iteration, num_floating_point_operations_so_far = train(
                ...
                process_non_loss_data_func, config, throughputs_list)

        print_datetime('after training is done')
        if throughputs_list:
            print('Average Throughput: {:.2f}'.format(sum(throughputs_list) / len(throughputs_list)))
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
                 ...
                 grad_norm, params_norm, num_zeros_in_grad, throughputs_list):
    ...
    if iteration % args.log_interval == 0:
        ...
        throughput = batch_size / elapsed_time_per_iteration
        throughputs_list.append(throughput)
        log_string += ' throughput (samples/sec): {:.2f} |'.format(throughput)
